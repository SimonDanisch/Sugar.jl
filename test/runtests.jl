...
@testset "round trips" begin
    @testset "control flow" begin
        func_expr = Sugar.get_func_expr(controlflow_1, Tuple{Int, Int}, gensym(:controlflow_1))
        try
    ...
    @testset "complex exp" begin
        ...
        func_expr = Sugar.get_func_expr(exp, Tuple{Complex64}, gensym(:exp_sugar))
        try
            round_tripped = eval(func_expr)
            for i = 1:10
                x = rand(Complex64)
    ...
    @testset "apply vararg" begin
        func_expr = Sugar.get_func_expr(apply_vararg2, Tuple{Int, Int, Int}, gensym(:varargtest))
        try
            round_tripped = eval(func_expr)
